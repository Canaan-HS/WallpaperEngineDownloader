...
def get_ext_groups(
    path: Path, exclude_folder: str | set[str] | tuple[str, ...] | list[str] = ""
) -> dict:
    """
    掃描指定路徑下的所有檔案，並依副檔名進行分組。

    此函式會忽略指定的資料夾（如果提供），然後將所有找到的檔案
    ...
    Args:
        path (Path): 掃描的根目錄路徑。
        exclude_folder (str | set[str] | tuple[str, ...] | list[str], optional):
            要排除的資料夾名稱。
            可傳入單一字串或多個資料夾名稱。
            預設為空字串，表示不排除任何資料夾。
    ...
    Example:
        ...
        {
            'txt': [
                PosixPath('test_dir/docs/c.txt'),
                PosixPath('test_dir/d.txt')
            ],
    ...
    if isinstance(exclude_folder, str):
        exclude_folder = {exclude_folder} if exclude_folder else set()
    else:
        exclude_folder = set(exclude_folder)

    for file in path.rglob("*"):

        # 是檔案, 且不是要排除的資料夾
        if file.is_file() and exclude_folder.isdisjoint(file.parts):
