...
- [.NET 10.0 執行庫](https://dotnet.microsoft.com/download/dotnet/10.0/runtime)
...
- 支援直接使用預設的 Steam 賬戶，無需登入即可下載。
- **QR Code 登入**
  支援使用 QR Code 登入，保存於本地供給之後下載使用。
  每過一段時間會話可能自動過期，需要重新掃描登入，切換回 QRCodeLogin 即可。
